...
通道允许 goroutine 通过使用信号语义相互通信。通道通过使用发送接收数据或通过识别单个通道上的状态变化来完成此信令。不要以通道是队列的想法来构建软件，而是专注于简化所需编排的信令和语义。
...
[Basic mechanics](example/basic_mechanics/basic_mechanics.go) ([Go Playground](https://play.golang.org/p/vG1rI7VteNH))  
[Tennis game](example/example2/example2.go) ([Go Playground](https://play.golang.org/p/PvFKD_tNwir))  
[Relay race](example/example3/example3.go) ([Go Playground](https://play.golang.org/p/OLdBCGUvzbx))  
[Fan out pattern](example/example4/example4.go) ([Go Playground](https://play.golang.org/p/zxzHAHIr3Xj))  
[Monitor running time](example/example5/example5.go) ([Go Playground](https://play.golang.org/p/vZ95XZuYVPF))
