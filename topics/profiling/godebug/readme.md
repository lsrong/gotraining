...
我们可以使用 GODEBUG 环境变量获取有关垃圾收集和调度程序的特定跟踪信息。该变量将导致运行时发出跟踪信息。 
...
查看逻辑处理器上的负载。我们只能看到可运行的 goroutine。 5 秒后，我们在跟踪中不再看到任何 goroutine。
...
没有办法在代码中具体识别发生泄漏的地方。我们可以验证是否存在内存泄漏以及哪些函数或方法产生最多的分配
...
设置 gctrace=1 会导致垃圾收集器在每次收集时向标准错误发出一行，总结收集的内存量和暂停的长度。设置 gctrace=2 会发出相同的摘要，但也会重复每个集合。此行的格式可能会发生变化:  
